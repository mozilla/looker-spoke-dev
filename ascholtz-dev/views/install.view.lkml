view: install {
  ...
  dimension: join_date {
    ...
    type: date
    sql: IF({% parameter previous_time_period %}, DATE(DATE_SUB(${submission_timestamp_date}, INTERVAL DATE_DIFF(DATE({% date_start date %}), DATE({% date_end date %}) + 1, DAY) DAY)), ${submission_timestamp_date}) ;;
  }
