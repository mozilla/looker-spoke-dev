...
explore: new_profile {
  ...
  join: prev_new_profile {
    ...
    sql_where: ${prev_new_profile.submission_timestamp_date} BETWEEN DATE_SUB({% date_start new_profile.submission_timestamp_date%}, INTERVAL DATE_DIFF(DATE({% date_start new_profile.submission_timestamp_date%}), DATE({% date_end new_profile.submission_timestamp_date%}), DAY) DAY)) DAY) AND DATE_SUB({% date_end new_profile.submission_timestamp_date%}, INTERVAL DATE_DIFF(DATE({% date_start new_profile.submission_timestamp_date%}), DATE({% date_end new_profile.submission_timestamp_date%}), DAY) DAY)) DAY)  ;;
  }
}
